    ...
    <div class="mb-6">
        <a href="{{ route('addresses.show', $address) }}"
           class="inline-flex items-center gap-1 text-sm text-gray-500 hover:text-gray-300 transition-colors">
            ...
            Back to {{ $address->alias }}
        </a>
        <h2 class="text-xl font-bold text-white mt-3">New device</h2>
        <p class="text-sm text-gray-500 mt-0.5">{{ $used }} of {{ $limit }} devices used in this property</p>
    </div>

    <div class="bg-[#111827] border border-gray-800 rounded-xl p-6 max-w-lg">
        <form action="{{ route('addresses.devices.store', $address) }}" method="POST" class="space-y-5">
            @csrf
            ...
            <div>
                ...
                <input type="text" id="name" name="name" value="{{ old('name') }}" autofocus
                       placeholder="E.g.: Living room light, Front camera"
                ...
                <div class="grid grid-cols-2 gap-2">
                    @foreach ($types as $value => $label)
                        <label class="relative flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-colors
                                      {{ old('type') === $value ? 'border-blue-600 bg-blue-900/20' : 'border-gray-700 bg-gray-800 hover:border-gray-600' }}">
