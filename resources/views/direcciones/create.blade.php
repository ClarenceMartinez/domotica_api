<x-layouts.app title="Nuevo inmueble">

    <div class="mb-6">
        <a href="{{ route('clientes.show', $cliente) }}" class="inline-flex items-center gap-1 text-sm text-gray-500 hover:text-gray-300 transition-colors">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
            </svg>
            Volver a {{ $cliente->nombre }}
        </a>
        <h2 class="text-xl font-bold text-white mt-3">Nuevo inmueble</h2>
    </div>

    <div class="bg-[#111827] border border-gray-800 rounded-xl p-6 max-w-lg">
        <form action="{{ route('clientes.direcciones.store', $cliente) }}" method="POST" class="space-y-5">
            @csrf
            ...
            <div>
                ...
                <input type="text" id="alias" name="alias" value="{{ old('alias') }}" autofocus
                       placeholder="Ej: Casa, Oficina, Local"
            ...
            <div>
                ...
                <select id="plan_id" name="plan_id"
                    ...
                    <option value="" class="text-gray-600">Seleccioná un plan</option>
